...
| **`npm run smoke` / `npm run qa:automated` / `npm run qa:isolated`** | **GitHub Actions** runs **`qa:isolated`** (**`smoke-gate-paths-sync`**, ephemeral **`server.js`** with **`IH35_SMOKE_GATE=1`** — same full gate); locally use **`smoke`** / **`qa:automated`** (**`qa:automated`** = **`smoke-gate-paths-sync`** + **`rule0:check`** + **`smoke`**) when a server is already up | Document port via **`SMOKE_BASE`** if not **3400**; **`qa:isolated`** picks a free port. **CI** sets **`SMOKE_TIMEOUT_MS=12000`** (**`.github/workflows/rule0-check.yml`**); locally optional **`SMOKE_TIMEOUT_MS`** — **README** (*Verification*). |
