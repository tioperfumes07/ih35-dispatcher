...
| **`scripts/system-smoke.mjs`** | Non-destructive HTTP smoke: **`HTML_PAGES`** includes **`/tracking.html`** (**`maintenance.html#tracking`** — **changelog 85**). |
...
| **24** | Connection verification | **Partial** — maintenance sidebar + **new strip** on satellites/index; **`.erp-connection-strip--ok|warn`** borders tokenized (**changelog 86**); not universal Samsara+QBO banner everywhere. |
