...
| **`public/settings.html`** | Tokens, toast host, `erp-ui.js`, toasts + busy, **`erpNotify`**, **`--color-bg-page`**, **connection strip** + `load` mount; focus + **`.err`** use **`var(--color-border-focus|focus-ring|semantic-error)`**. |
...
| **2** | App shell | **Partial** — maintenance **`erp-master`** + **Lists** (**▤**) icon opens **`#catalog`**; dispatch nav links to **`/maintenance.html#catalog`**; spec copy/dimensions **Future**. |
