...
| **15** | Filter bar | **Partial** — security alerts + **Lists & catalogs → Service types (DB)** search row + **Reports → Maintenance detailed** filter strip + **Accounting → Fuel expenses** ledger **`mr-filter-bar`**; not every table. |
