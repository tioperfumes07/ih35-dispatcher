...
| **`public/css/erp-master-spec-2026.css`** | Toasts, busy spinners, help tips, maintenance/dispatch/fuel/banking/settings hooks, **`.erp-connection-strip*`**, **`body.erp-maintenance #authBanner.erp-auth-banner`** (login-required strip, Rule **0**), dispatch main-column token bridge; **Reports** hub **≤900px** toolbar + main-column gap tighten (Rule **1**). |
...
|------|--------|------------------|
| **0** | Design tokens | **Partial** — `design-tokens.css` + satellite **`--color-bg-page`** + **`board-nav.css`** token bridges + **QBO sync bar** + **`#authBanner`** **`pill-*`** fallbacks; **reports** **`renderReportsAll`** / **`renderRepMaintSpendByUnitTable`** generated copy uses **`--color-text-label`** / **`--color-semantic-error`**; full migration of every legacy var not done. |
...
| **12** | Maintenance layout | **Partial / evolving** — **Lists** shell target + **Service types (DB)** filter/pager in **Lists & catalogs**. |
| **13** | Accounting board | **Partial** — KPI strip; **`#authBanner`** token strip for 401 hints. |
...
| **17** | Reports | **Partial** — hub + **Executive overview** title **`erp-help-tip`**; **QBO / sync / IFTA** body intros shortened in **`renderReportsAll`**; **TMS** + **spend-by-unit** empty states aligned with headline + muted second line. |
...
| **22** | “?” tips | **Done (pattern)** + **samples** (incl. **reports** settlement, **Executive overview**, shorter **QBO/sync/IFTA** intros, **TMS** + **spend-by-unit** empty two-liners); many maintenance paragraphs remain. |
