...
|------|--------|------------------|
| **0** | Design tokens | **Partial** — `design-tokens.css` + satellite **`--color-bg-page`** + **`board-nav.css`** token bridges + **QBO sync bar** + **`#authBanner`** **`erp-auth-banner`** on **maintenance** + **banking**; **settlement** TMS box + rollback sub-heads use **`--color-border`** / **`--color-bg-card`**; **reports** generated copy uses **`--color-text-label`** / **`--color-semantic-error`**; full migration of every legacy var not done. |
...
| **13** | Accounting board | **Partial** — KPI strip; **`#authBanner`** **`erp-auth-banner`** token strip on **maintenance** + **banking** for 401 hints. |
...
| **22** | “?” tips | **Done (pattern)** + **samples** (incl. **reports** settlement, **Executive overview**, shorter **QBO/sync/IFTA** intros, **TMS** + **spend-by-unit** empty two-liners, **rollback** + **driver pay settlements** titles); many maintenance paragraphs remain. |
