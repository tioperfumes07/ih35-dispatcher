...
| **`public/dispatch.html`** | Tokens, toast host, `erp-ui.js`, intro + stops help tips, `erpWithBusy` / `showToast` on refresh, QBO catalog, save, uploads, PDF, auto miles, row QBO, quick-add, `patchStatus`, escaped `showMsg`, `loadTab(rethrow)` for manual refresh; load-docs list + sidebar **QBO alert** + page `<style>` chrome (**stops**, modal, miles table, buttons) use **`var(--color-*)`**. |
