...
| **15** | Filter bar | **Partial** — security alerts + **Lists & catalogs → Service types (DB)** search row + **Reports → Maintenance detailed** + **Reports → Settlement** trip lookup + **Accounting → Fuel expenses** + **Expense history** + **Pay bills** composer + **Recent bill payments** log **`mr-filter-bar`**; not every table. |
