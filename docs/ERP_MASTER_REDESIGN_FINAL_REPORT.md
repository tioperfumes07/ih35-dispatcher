...
| **23** | Pagination | **Partial** — banking/settings + maintenance **shop queues**, **fuel expense**, **expense history**, **saved WO/AP cards**, **maintenance fleet table**, **Safety / HOS** (HOS table + active / in-service + assignments), **tracking** (map unit cards, assets grid, HOS drivers, **idle snapshot**), **bill payment log**, **reports settlement** (index + load lines), and **reports → Maintenance detailed** record cards wired; upload center “recent” lists stay capped at 10; other long tables remain. |
