...
| **Rule 22 (samples)** | Dispatch board + banking sample + maintenance **accident** + **tire** WO + **reports settlement** intros + **`fuel.html`** **Selected Unit** title folded into **`erp-help-tip`**. |
...
| **15** | Filter bar | **Partial** — security alerts + **Lists & catalogs → Service types (DB)** search row + **Reports → Maintenance detailed** + **Reports → Settlement** trip lookup + **Accounting → Fuel expenses** (ledger + manual fuel header) + **Expense history** + **Pay bills** composer + **Recent bill payments** log + **`fuel.html`** diesel purchase row **`mr-filter-bar`**; not every table. |
...
| **22** | “?” tips | **Done (pattern)** + **samples** (incl. **reports** settlement, **Executive overview**, shorter **QBO/sync/IFTA** intros, **TMS** + **spend-by-unit** empty two-liners, **rollback** + **driver pay settlements** + **QuickBooks Live Master** titles, **`fuel.html`** **Selected Unit**); many maintenance paragraphs remain. |
