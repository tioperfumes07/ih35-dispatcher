...
- **2026-04-18 — GitHub Actions Rule 0:** **[`.github/workflows/rule0-check.yml`](../.github/workflows/rule0-check.yml)** originally ran **`npm ci`** + **`npm test`** (`**rule0:check**` only). **Superseded 2026-04-19+:** workflow runs **`npm run qa:isolated`** (Rule 0 + HTTP smoke); job env includes **`SMOKE_TIMEOUT_MS=12000`**, **`permissions: contents: read`**, **concurrency**, **timeout**; **`README`**, **FINAL_REPORT** § P0, **POST_RELEASE_CHECKLIST** §0, **PARALLEL_AGENT** §8, **ARCHITECTURE**, and **AGENT_COORDINATION** document smoke (**`Content-Type`** on JSON 404), **`qa-with-server`** (**SIGINT**/**SIGTERM**), and local **`SMOKE_*`** env vars. **`npm test`** still maps to **`rule0:check`** for offline-only checks.
