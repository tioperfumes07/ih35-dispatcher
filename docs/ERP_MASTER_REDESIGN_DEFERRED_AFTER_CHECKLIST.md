...
- **Lists and catalogs** — **partial (changelogs 224 + 321 + 348 + 362 + 381):** **Service types** (**224**); **QuickBooks**, **Vendors**, **Operational**, **Fleet** sub-panel `**mini-note**` / `**erp-help-tip**` trims (**321**); **`#section-catalog`** panel + all sub-tab `**erp-help-tip**` bodies tightened again (**348**); **362** shortens **QBO** / **Vendors &amp; drivers** / **Operational** / **Fleet** catalog copy, **`erpSbAdmin`** lists blurb, **`renderErpModuleSidebar`** accounting Connections + reports hub `**mini-note**`, and **`erpNewPlaceholder`** drawer; **381** re-trims **`#section-catalog`** sub-panel tips + **`fuel.html`** planner / Relay blurbs. Optional JS-rendered catalog strings if product asks.
...
- **2026-04-18 — Lists &amp; catalogs + fuel board copy:** **`#section-catalog`** sub-panel **`erp-help-tip`** / **`mini-note`**; **`fuel.html`** roadmap, write banner, **Selected Unit** **`?`**, diesel ledger, Relay callout — Rule 22 (**changelog 381**).
- **2026-04-18 — Banking + settings satellites:** **`banking.html`** lead + CSV **`?`**; **`settings.html`** Users / Employees **`?`** + employees **`mini-note`** — Rule 22 (**changelog 389**).
...
- **2026-04-18 — Fast path (operational priority):** Team explicitly **defers Rule 10** (shared StandardExpenseLines-style module — no change to current maintenance HTML/JS cost lines) and **DEFERRED §2 optional** Rule 22 copy / dense **`innerHTML`** cleanup until after go-live. **Dispatch** parity is **not** prioritized until product needs it. Next work: keep **`npm run rule0:check`** + **`npm run smoke`** green, then run **[`ERP_MASTER_REDESIGN_POST_RELEASE_CHECKLIST.md`](./ERP_MASTER_REDESIGN_POST_RELEASE_CHECKLIST.md)** (manual QBO / Samsara / auth). **`npm run report:erp`** refreshed same day.
- **2026-04-18 — `erpMountConnectionStrip` non-OK HTTP (Rule 24):** **`public/js/erp-ui.js`** — if **`GET /api/qbo/status`** returns a non-2xx status, the strip shows **muted** “QuickBooks: could not load status.” instead of mis-reading the body as “not configured.” Aligns with **FINAL_REPORT** §6 (graceful degradation if status ever returns errors).
- **2026-04-18 — `npm run qa:automated` + strip DRY:** **`package.json`** adds **`qa:automated`** (`**rule0:check**` then **`smoke`** — **later amended** so **`qa:automated`** runs **`smoke-gate-paths-sync`** first, matching the **`qa:isolated`** prelude; see **README** / **POST_RELEASE_CHECKLIST** §0). **`POST_RELEASE_CHECKLIST`** §0 mentions it. **`erpMountConnectionStrip`** uses a single **`stripLoadFailed`** constant for non-2xx + **`catch`** paths.
- **2026-04-18 — README verification:** Root **[`README.md`](../README.md)** gains **Verification (automated)** — **`rule0:check`**, server **`PORT`** / default **3400**, **`smoke`** / **`qa:automated`**, **`SMOKE_BASE`** for non-default ports.
- **2026-04-18 — Smoke script + checklist:** **`scripts/system-smoke.mjs`** prints a trailing **Smoke target** line (suppress with **`SMOKE_QUIET=1`**); header comment documents **`qa:automated`**. **`POST_RELEASE_CHECKLIST`** §0 automated smoke row aligned with **`SMOKE_BASE`** / **`SMOKE_QUIET`**.
- **2026-04-18 — GitHub Actions Rule 0:** **[`.github/workflows/rule0-check.yml`](../.github/workflows/rule0-check.yml)** originally ran **`npm ci`** + **`npm test`** (`**rule0:check**` only). **Superseded 2026-04-19+:** workflow runs **`npm run qa:isolated`** (**`smoke-gate-paths-sync`**, Rule 0 + HTTP smoke, ephemeral **`server.js`** with **`IH35_SMOKE_GATE=1`**); job env includes **`SMOKE_TIMEOUT_MS=12000`**, **`permissions: contents: read`**, **concurrency**, **timeout**; **`README`**, **FINAL_REPORT** § P0, **POST_RELEASE_CHECKLIST** §0, **PARALLEL_AGENT** §8, **ARCHITECTURE**, and **AGENT_COORDINATION** document smoke (**`Content-Type`** on JSON 404, **`GET /api/pdf/__smoke__`**), **`qa-with-server`** (**SIGINT**/**SIGTERM**), and local **`SMOKE_*`** / **`.env.example`** (**`IH35_SMOKE_GATE`** note). **`npm test`** still maps to **`rule0:check`** for offline-only checks.
- **2026-04-18 — Post-release §0 CI row:** **[`ERP_MASTER_REDESIGN_POST_RELEASE_CHECKLIST.md`](./ERP_MASTER_REDESIGN_POST_RELEASE_CHECKLIST.md)** §0 adds a checkbox that **Rule 0** GitHub Actions is green on the shipped commit.
- **2026-04-18 — Architecture cross-link:** **[`ARCHITECTURE.md`](./ARCHITECTURE.md)** gains **ERP shell verification (master redesign)** — pointers to **`README`** verification, **POST_RELEASE_CHECKLIST**, **CI** workflow, **FINAL_REPORT** / **STATUS**.
- **2026-04-18 — README ↔ architecture:** Root **[`README.md`](../README.md)** architecture line references that section so readers find verification from the repo landing page.
- **2026-04-18 — Smoke script footer:** **`scripts/system-smoke.mjs`** prints resolved **`SMOKE_BASE`** / default URL + pointer to **`npm run qa:automated`** on success; **`SMOKE_QUIET=1`** skips that line. Header comment documents **`qa:automated`**.
