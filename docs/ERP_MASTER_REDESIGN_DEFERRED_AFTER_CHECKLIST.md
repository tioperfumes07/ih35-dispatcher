...
- **2026-04-18 — `npm run qa:automated` + strip DRY:** **`package.json`** adds **`qa:automated`** (`**rule0:check**` then **`smoke`**). **`POST_RELEASE_CHECKLIST`** §0 mentions it. **`erpMountConnectionStrip`** uses a single **`stripLoadFailed`** constant for non-2xx + **`catch`** paths.
