...
- **Listen address:** `server.js` binds **`0.0.0.0:<PORT>`** so **`http://127.0.0.1:<PORT>`** (smoke’s default) connects on hosts where Node would otherwise listen IPv6-only; use **`SMOKE_BASE`** if you need another origin.
