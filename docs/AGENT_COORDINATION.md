...
- **`npm run qa:isolated` + signals:** **`scripts/qa-with-server.mjs`** registers **SIGINT**/**SIGTERM** so **Ctrl+C** or stop signals **`SIGTERM`** the child **`server.js`** and any in-flight **`rule0:check`** / **`smoke`** Node child, then **`process.exit`** (**130** / **143**) — see root **`README.md`** (*Verification*) and **`ARCHITECTURE.md`** (*ERP shell verification*).
