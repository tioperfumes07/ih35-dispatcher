...
- **`scripts/rule-zero-agent-b.mjs`**, **`scripts/rule-zero-agent-b-check.mjs`**, **`scripts/system-smoke.mjs`**, **`scripts/smoke-gate-paths-sync.mjs`**, **`scripts/qa-with-server.mjs`**, **`package.json`** test/qa scripts — if smoke expectations change, sync with Agent A / Agent 1 as needed. After changing **`CRITICAL`** or **`SMOKE_GATE_API_PATHS`**, run **`npm run smoke:gate-sync`** ( **`qa:isolated`** and **`qa:automated`** run it automatically). **`npm run smoke`** (server up; optional **`SMOKE_BASE`**, default **`http://localhost:<PORT>`**) GETs ERP HTML shells (including **`/tracking.html`** — redirect target + **`viewport-fit=cover`**) plus static assets with stable header needles: **`/css/design-tokens.css`**, **`/css/app-theme.css`**, **`/css/erp-master-redesign.css`**, **`/css/erp-master-spec-2026.css`**, **`/css/maint-accounting-ui-2026.css`**, **`/css/board-nav.css`**, **`/js/erp-ui.js`**, **`/js/board-nav.js`**; **`GET /api/__smoke_not_found__`** (auth-exempt) asserts unknown API paths return **404** with **`Content-Type`** including **`application/json`** and JSON (`error`, `path`); **`GET /api/pdf/__smoke__`** (auth-exempt) asserts **200** + **`application/pdf`** + **`%PDF`** body ( **`pdfkit`** ). Rule 0 stack guard reuses cached bodies for **`app-theme`**, **`maint-accounting`**, **`maintenance.html`**. **`npm run qa:automated`** = **`smoke-gate-paths-sync`** + **`rule0:check`** + smoke (existing listener). **`npm run qa:isolated`** = **`smoke-gate-paths-sync`** + **`rule0:check`** + smoke on a child **`server.js`** on a free port (**`IH35_SMOKE_GATE=1`** on that child). **CI** ([**`.github/workflows/rule0-check.yml`**](../.github/workflows/rule0-check.yml)) runs **`npm run qa:isolated`** on push/PR; **`npm test`** remains **`rule0:check`** for a quick offline-only guard.
- **`npm run qa:isolated` + signals:** **`scripts/qa-with-server.mjs`** registers **SIGINT**/**SIGTERM** so **Ctrl+C** or stop signals **`SIGTERM`** the child **`server.js`** and any in-flight **`rule0:check`** / **`smoke`** Node child, then **`process.exit`** (**130** / **143**). The child server gets **`IH35_SMOKE_GATE=1`** so **`system-smoke.mjs`** JSON GETs match **`server.js`** **`SMOKE_GATE_API_PATHS`** without a session when auth is on — see root **`README.md`** (*Verification* → **`qa:isolated`**).
- **`.github/workflows/rule0-check.yml`** — runs **`npm run qa:isolated`**; job env sets **`SMOKE_TIMEOUT_MS=12000`** for HTTP fetches in **`system-smoke.mjs`** (default **8000** ms when unset). For local **`SMOKE_TIMEOUT_MS`** / **`SMOKE_BASE`** / **`SMOKE_QUIET`**, see root **README** (*Verification*). CI summary: **README** (*Verification* → CI) and **`ARCHITECTURE.md`** (*ERP shell verification* → CI).
- **Dependency audit:** Run **`npm audit`** periodically after dependency changes. Spreadsheet parsing uses **`@e965/xlsx`** (SheetJS-compatible); see root **README** (*Dependency audit*). Replacing it touches **`server.js`** — coordinate before swapping.
...
4. **Run verification yourself:** When a task needs **`npm run rule0:check`**, **`npm run qa:isolated`**, **`npm run smoke`**, or similar, the agent should execute those commands in this session with a long enough wait (e.g. **`block_until_ms`** / **120000** for **`qa:isolated`**) so the gate finishes without asking the human to click **Run in Background** or approve a terminal step. (If the IDE still surfaces approval UI for very long jobs, that is editor behavior outside this repo—use backgrounded tooling only when the product requires it, not as the default handoff.)
