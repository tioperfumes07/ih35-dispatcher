...
- [ ] **Automated smoke:** With server up, `npm run smoke` → **`node scripts/system-smoke.mjs`** completes without failures (default **`http://localhost:<PORT>`**, same **`PORT`** as the server / default **3400**); smoke includes **`GET /api/__smoke_not_found__`** (**404**, **`Content-Type`** includes **`application/json`**, JSON **`{ error, path }`**) and **`GET /api/pdf/__smoke__`** (minimal **PDF** / **`pdfkit`**). Shortcut: `npm run qa:automated` runs **smoke-gate-paths-sync**, then **rule0**, then **smoke** (requires server already listening). Alternative: **`npm run qa:isolated`** runs **`scripts/smoke-gate-paths-sync.mjs`**, then **rule0** + **smoke** on a temporary `server.js` on a free port (**`IH35_SMOKE_GATE=1`** on that child so JSON smoke GETs succeed when ERP login is required; no manual start; avoids stale listeners on **3400**). If the listen port or host differs from smoke’s default, set **`SMOKE_BASE`** (see root **`README.md`** — Verification). Optional: **`SMOKE_QUIET=1`** hides the trailing “Smoke target” line on success.
- [ ] **CI (GitHub):** Workflow **[`.github/workflows/rule0-check.yml`](../.github/workflows/rule0-check.yml)** — **`npm ci`** + **`npm run qa:isolated`** (**smoke-gate-paths-sync**, Rule 0, HTTP smoke on an ephemeral server) is **green** on the commit/tag you release; the job sets **`SMOKE_TIMEOUT_MS=12000`** (see root **`README.md`** — Verification → CI).
