...
- [ ] **Automated smoke:** With server up, `npm run smoke` → **`node scripts/system-smoke.mjs`** completes without failures (default **`http://localhost:<PORT>`**, same **`PORT`** as the server / default **3400**); smoke includes **`GET /api/__smoke_not_found__`** (**404**, **`Content-Type`** includes **`application/json`**, JSON **`{ error, path }`**). Shortcut: `npm run qa:automated` runs **rule0** then **smoke** (requires server already listening). Alternative: **`npm run qa:isolated`** runs **rule0** + **smoke** against a temporary `server.js` on a free port (no manual start; avoids stale listeners on **3400**). If the listen port or host differs from smoke’s default, set **`SMOKE_BASE`** (see root **`README.md`** — Verification). Optional: **`SMOKE_QUIET=1`** hides the trailing “Smoke target” line on success.
- [ ] **CI (GitHub):** Workflow **[`.github/workflows/rule0-check.yml`](../.github/workflows/rule0-check.yml)** — **`npm ci`** + **`npm run qa:isolated`** (Rule 0 + smoke on an ephemeral server) is **green** on the commit/tag you release; the job sets **`SMOKE_TIMEOUT_MS=12000`** (see root **`README.md`** — Verification → CI).
