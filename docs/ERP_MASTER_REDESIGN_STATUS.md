...
**Overall completion: ~60.4%** toward the **documented intent** of Rules **0–24** in this repo (not pixel-perfect spec parity). Figures are for **planning and continuity**; they are judgment-based, not a contract metric. (Average of the per-rule fractions in the table below ÷ 25 ≈ **0.604** → **60.4%**; rounded headline **~60.4%**.)
...
| 2 | App shell | 0.50 | **`erp-master`** + **Lists** (**▤**) icon opens **`#catalog`**; dispatch + **settings** + hub **`board-nav`** strip; **`index.html`** company hub header **`erp-help-tip`** for cards + strip + QBO line (**changelog 55**); spec dimensions/copy deferred |
...
| 15 | Filter bar | 0.62 | Several tables + **Security alerts** + **Lists & catalogs → Service types (DB)** + **Reports → Maintenance detailed report** + **Reports → Settlement** trip lookup + **Accounting → Fuel expenses** (ledger + **manual fuel / DEF** header grid) + **Expense history** (grid + date presets in **`mr-filter-bar__right`**) + **Pay bills** composer + **Recent bill payments** log filters + **Fuel & route** satellite **Diesel purchase** + **Bank & matching** steps **1** (history window) + **3** (batch id + suggestions) + **Settings → Employees** search row (**`body.fuel-board`**, **`body.banking-page`**, **`body.settings-page`** in **`erp-master-spec-2026.css`**) (**changelogs 48–54**); not universal |
| 16 | Safety / HOS | 0.55 | Tables + **Rule 23** pagers on Safety + idle snapshot |
| 17 | Reports | 0.62 | Hub + settlement; **Executive overview** panel title + **`erp-help-tip`**; **TMS**, **QBO**, **sync**, **IFTA**, **Maintenance spend by unit**, **Team & security**, **Maint detailed** tab titles + **`erp-help-tip`**; **Settlement** trip cost rollup load # + actions use **`mr-filter-bar`** (**changelog 51**); **Settlement** TMS trip box uses **`var(--color-border|bg-card)`**; **Driver pay settlements** title **`erp-help-tip`** + short body lines; **QBO / sync / IFTA** bodies use shorter intro copy (**Rule 22**); **Executive overview** timestamp line + **TMS** summary / empty / error copy use token-muted (**Rule 0**); **Maint spend by unit** empty state two-line pattern; **Maint spend by unit** + **Maint detailed** bodies paginated (**Rule 23**); “all on one page” partial |
...
**Rolling average (above table):** sum of fractions ÷ 25 ≈ **0.604** (**60.4%**; headline **~60.4%**). When reporting updates, **revise fractions** (not the formula) as work lands, then re-average.
...
- **Partial (this pass):** Added `public/css/design-tokens.css` with spec-aligned **additional** `:root` variables (`--color-*`, **`--color-semantic-*`**, `--btn-*`, `--pill-*`, spacing, dimensions). Linked **before** `app-theme.css` on maintenance, dispatch, fuel, banking, settings. **Satellite page shells** use **`background: var(--color-bg-page, var(--bg))`** on **banking**, **settings**, and **fuel** (company hub stays dark **`--bg`** only). **Banking**, **settings**, **dispatch**, and **maintenance** inline / `<style>` blocks increasingly use **`var(--color-*|pill-*|btn-danger-*|focus-ring|semantic-*)`** (see changelog **33**). **`fuel.html`** inline layout CSS maps panels, badges, autocomplete, write-secret banner, and relay pointer box to **`var(--color-*|pill-*|btn-info-*|shadow-dropdown)`** (**changelog 53**). **`banking.html`** panel header gradient + default button fills; **`settings.html`** primary button text on accent uses **`var(--color-bg-card)`** (**changelog 54**). **`index.html`** hub inline CSS uses **`var(--color-nav-bg|shadow-dropdown|pill-*|btn-info-*|color-text-placeholder|font-spec-sans)`** fallbacks on the dark shell (**changelog 55**). **`#authBanner.erp-auth-banner`** on **maintenance** and **banking** uses the same token-backed warning strip in **`erp-master-spec-2026.css`** (changelog **43** + **46**). **`renderReportsAll`** overview timestamp, **TMS** loaded summary, and **`renderRepMaintSpendByUnitTable`** empty secondary line use **`var(--color-text-label)`**; **TMS** errors use **`var(--color-semantic-error)`** (changelog **45**). **Accounting** `#maintIntegrationStrip` + **`#maintPostingPreflight`** + **Lists & catalogs** panel subtitle use design-token fallbacks (**changelog 48**).
...
- **Partial:** Tabs and import flows exist; panel-head **?** on **Upload center**; **?** + one-line intro on **every** upload subpanel (**Bank**, **Comdata**, **Fuel**, **AP**, **Maint history**, **Other**, **Connections**). Sub-tab buttons are ordered **A–Z** by label (**Bank CSV** … **Other**). “Every tab fully functional” should be verified with manual tests and real files.
...
- **Done (pattern):** Global **`.erp-help-tip*`** styles; **`erpHelpTipToggle`** + click/Escape close in **`erp-ui.js`**. Banking includes a sample tip. Dispatch board uses compact copy + tips. **Maintenance:** **accident** WO **Cost breakdown** + **tire** WO “same invoice / multiple positions” + **shop → Parts** queue intro each use a short line + **`erp-help-tip`** panel; **reports → Executive overview** panel title + **?** (Refresh vs QBO sync); **reports → Settlement / P&L** intro + trip rollup panel use the same pattern; **reports → Settlement → Driver pay settlements** title + **?**; **Accounting → QuickBooks rollback & import safety** title + **?**; **Fleet → Vehicles by shop / location context** title + **?**; **reports** tabs **TMS**, **QBO**, **sync**, **IFTA**, **maintenance spend**, **detailed**, **team** + **Lists & catalogs** (panel title + **all five** sub-tab intros) + **?**; **Upload center** (prior pass). **`fuel.html`** **Selected Unit** panel title + **?** explains satellite diesel ledger vs **Maintenance → Fuel expenses** (**changelog 53**). **`settings.html`** **Employees** title + **?** distinguishes internal directory from **Lists & catalogs** QBO vendors (**changelog 54**). **`index.html`** company hub header + **?** (**changelog 55**). Many other long **`mini-note`** blocks remain for future passes.
