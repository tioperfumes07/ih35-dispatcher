...
**Overall completion: ~60.2%** toward the **documented intent** of Rules **0–24** in this repo (not pixel-perfect spec parity). Figures are for **planning and continuity**; they are judgment-based, not a contract metric. (Average of the per-rule fractions in the table below ÷ 25 ≈ **0.602** → **60.2%**; rounded headline **~60.2%**.)
...
| 15 | Filter bar | 0.61 | Several tables + **Security alerts** + **Lists & catalogs → Service types (DB)** + **Reports → Maintenance detailed report** + **Reports → Settlement** trip lookup + **Accounting → Fuel expenses** (ledger + **manual fuel / DEF** header grid) + **Expense history** (grid + date presets in **`mr-filter-bar__right`**) + **Pay bills** composer + **Recent bill payments** log filters + **Fuel & route** satellite **Selected Unit → Diesel purchase** row (**`erp-master-spec-2026.css`** scopes **`mr-filter-bar`** to **`body.fuel-board`**) wrapped in **`mr-filter-bar`** + **`mr-filter-bar__grow`** / **`__actions`** (**changelogs 48–53**); not universal |
...
**Rolling average (above table):** sum of fractions ÷ 25 ≈ **0.602** (**60.2%**; headline **~60.2%**). When reporting updates, **revise fractions** (not the formula) as work lands, then re-average.
