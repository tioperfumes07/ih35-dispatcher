...
| 15 | Filter bar | 0.59 | Several tables + **Security alerts** + **Lists & catalogs → Service types (DB)** + **Reports → Maintenance detailed report** + **Reports → Settlement** trip lookup row + **Accounting → Fuel expenses** + **Expense history** (grid + date presets in **`mr-filter-bar__right`**) + **Pay bills** composer + **Recent bill payments** log filters wrapped in **`mr-filter-bar`** + **`mr-filter-bar__grow`** (**changelogs 48–51**); not universal |
...
- **Partial:** Filter patterns exist on several tables (see spec comments in `erp-master-spec-2026.css`). **Maintenance → Security alerts** uses **`mr-filter-bar`** + **`mr-filter-bar__grow`** / **`__right`** for search, window, severity, and **Refresh alerts**. **Lists & catalogs → Service types (DB)** adds **`mr-filter-bar`** + live name filter for the Postgres catalog admin table. **Reports → Maintenance detailed report** wraps unit/date/type/search controls in **`mr-filter-bar`** + **`mr-filter-bar__grow`**. **Accounting → Fuel expenses** ledger filters, **Expense history** date/source/search row + preset buttons (**`mr-filter-bar`** + **`__grow`** / **`__right`**), **Reports → Settlement** trip **Load / invoice #** + **Run lookup** / **Download CSV** row, and **Pay bills** vendor/payment composer + **Recent bill payments** log filters use the same **`mr-filter-bar`** shell (**changelogs 48–51**). Not every table has full chip + pager parity.
