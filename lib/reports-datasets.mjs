  ...
  switch (id) {
      ...
      return {
        ...
        meta: {
          ...
          disclaimer: `Fleet average pace: ${fleetAvg.toLocaleString(
            'en-US'
          )} mi per truck per month. PM colors: green if miles remaining > 8,000; blue if 3,000–8,000; amber if 0–3,000; red if overdue. Weeks ≈ miles remaining ÷ (${(fleetAvg / 4.33).toFixed(0)} mi/wk). Catalog intervals use ROUND(miles ÷ fleet avg), minimum 1 month.`
        }
      };
    }
    ...
    case 'a7-battery-history': {
      ...
      for (const wo of activeWorkOrders(erp)) {
        ...
        for (const ln of wo.lines || []) {
          const pos = String(ln.partPosition || ln.tirePosition || ln.tirePositionText || '').trim() || '—';
          rows.push({
            date: sliceIso(wo.serviceDate),
            unit: wo.unit || '',
            position: pos,
