...
/**
 * Classify spend in a WO group to pick a dominant location-type pill (UI + section accent).
 * @param {object[]} list
 * @returns {{ locationPill: string, pillTone: 'green'|'blue'|'amber'|'purple'|'gray' }}
 */
function dominantServiceLocationPill(list) {
  const buckets = { internal: 0, external: 0, roadside: 0, dealer: 0, other: 0 };
  for (const wo of list || []) {
    const t = String(wo.repairLocationType || '').toLowerCase();
    const lab = String(wo.repairLocationLabel || '').toLowerCase();
    const cost = woLineTotal(wo);
    if (t === 'in-house' || t === 'internal') buckets.internal += cost;
    else if (t === 'road-service' || t === 'roadside' || t === 'road_side') buckets.roadside += cost;
    else if (t === 'dealer' || lab.includes('dealer') || lab.includes('oem')) buckets.dealer += cost;
    else if (
      t === 'external' ||
      t === 'parts-purchase' ||
      t === 'over-the-road' ||
      ...
      buckets.external += cost;
    else buckets.other += cost;
  }
  const order = ['internal', 'external', 'roadside', 'dealer', 'other'];
  let best = 'external';
  let bestV = -1;
  ...
  if (bestV <= 0 && list && list[0]) {
    const wo = list[0];
    const t = String(wo.repairLocationType || '').toLowerCase();
    const lab = String(wo.repairLocationLabel || '').toLowerCase();
    if (t === 'in-house' || t === 'internal') return { locationPill: 'Internal', pillTone: 'green' };
    if (t === 'road-service' || t === 'roadside' || t === 'road_side') return { locationPill: 'Roadside', pillTone: 'amber' };
    if (t === 'dealer' || lab.includes('dealer') || lab.includes('oem')) return { locationPill: 'Dealer', pillTone: 'purple' };
    if (t === 'external' || t === 'parts-purchase' || t === 'over-the-road')
      return { locationPill: 'External', pillTone: 'blue' };
  }
  const map = {
    internal: { locationPill: 'Internal', pillTone: 'green' },
    external: { locationPill: 'External', pillTone: 'blue' },
    roadside: { locationPill: 'Roadside', pillTone: 'amber' },
    dealer: { locationPill: 'Dealer', pillTone: 'purple' },
    other: { locationPill: 'Other', pillTone: 'gray' }
  };
  return map[best] || map.external;
}
...
export function primaryServiceType(wo) {
  const lines = wo.lines || [];
...
function queryWithoutPositionKeys(query) {
  const q = { ...query };
  delete q.position;
  delete q.positions;
  return q;
...
/** Same token list semantics as `workOrderPassesReportFilters` position logic (for line-level row filters). */
function positionLineFilterTokens(query) {
  let posFilters = parseMultiParam(query, 'positions', 'position');
  if (!posFilters.length && query.position) posFilters = [String(query.position).trim()].filter(Boolean);
  return posFilters.map(p => String(p).trim().toLowerCase()).filter(Boolean);
}

function lineMatchesPositionLineTokens(posRaw, tokens) {
  if (!tokens.length) return true;
  const pos = String(posRaw || '').trim().toLowerCase();
  return tokens.some(pf => pf && (pos === pf || pos.includes(pf)));
}
...
function serviceCategoryRollupLabel(serviceType) {
  const s = String(serviceType || '').toLowerCase();
  if (s.includes('brake')) return 'Brakes';
  if (s.includes('tire') || s.includes('wheel')) return 'Tires / wheels';
  if (s.includes('oil') || s.includes('filter') || s.includes('lubric') || s.includes('coolant') || s.includes('engine'))
    return 'Engine / fluids';
  if (s.includes('inspection') || s.includes('dot')) return 'Inspections';
  if (s.includes('reefer') || s.includes('liftgate')) return 'Reefer / liftgate';
...
export function workOrderPassesReportFilters(wo, query, ctx = {}) {
  ...
  if (locCats.length) {
    ...
    const matchCat = c => {
      ...
      if (x === 'internal' || x === 'internal_shop')
        return typ === 'in-house' || typ === 'internal' || lab.includes('yard') || lab.includes('in-house');
      ...
      if (x === 'dealer' || x === 'oem')
        return t === 'dealer' || lab.includes('dealer') || lab.includes('oem') || svcBlob.includes('dealer');
      if (x === 'other')
        return typ === 'other' || typ === 'unknown' || (!typ && !lab) || lab.includes('other');
  ...
  if (posFilters.length) {
    const lines = wo.lines || [];
    const lowered = posFilters.map(p => String(p).trim().toLowerCase()).filter(Boolean);
    const hit = lines.some(l => {
      const pos = String(l.tirePosition || l.tirePositionText || l.partPosition || '')
        .trim()
        .toLowerCase();
      return lowered.some(pf => pf && (pos === pf || pos.includes(pf)));
    });
    if (!hit) return false;
  }
  ...
  if (faultQ.length) {
    ...
    const ok = faultQ.some(x => {
      ...
      if (q === 'at_fault' || q === 'at fault') return f.includes('at fault') || f.includes('at-fault') || f === 'at_fault';
      if (q === 'not_at_fault' || q === 'not at fault')
        return f.includes('not') && f.includes('fault') && !f.includes('at fault');
      return f === q || f.includes(q);
    });
    if (!ok) return false;
  }

  const dotF = parseMultiParam(query, 'dotReportable', 'accidentDotReportable');
  if (dotF.length) {
    const raw = wo.accidentDotReportable != null ? wo.accidentDotReportable : wo.dotReportable;
    const isY = raw === true || String(raw).toLowerCase() === 'y' || String(raw).toLowerCase() === 'yes';
    const ok = dotF.some(x => {
      const q = String(x).trim().toLowerCase();
      if (q === 'y' || q === 'yes' || q === 'true' || q === '1') return isY;
      if (q === 'n' || q === 'no' || q === 'false' || q === '0') return !isY;
  ...
  const insF = parseMultiParam(query, 'insuranceClaim', 'insuranceClaimFilter');
  if (insF.length) {
    const claim = String(wo.insuranceClaimNumber || '').trim();
    const has = claim.length > 0;
    const ok = insF.some(x => {
      const q = String(x).trim().toLowerCase();
      if (q === 'has' || q === 'has_claim' || q === 'yes') return has;
      if (q === 'no' || q === 'no_claim' || q === 'none') return !has;
      return false;
    });
    if (!ok) return false;
  }

  const fleetTypes = parseMultiParam(query, 'fleetTypes', 'fleetType');
  if (fleetTypes.length) {
    const ft = String(wo.fleetType || wo.vehicleType || wo.assetCategory || wo.assetType || '')
      .trim()
      .toLowerCase();
    if (!ft) return false;
    if (!fleetTypes.some(x => ft === String(x).trim().toLowerCase() || ft.includes(String(x).trim().toLowerCase()))) {
      return false;
    }
  }

  const defectsOnly = String(query.defectsOnly || '').trim().toLowerCase();
  if (defectsOnly === '1' || defectsOnly === 'true' || defectsOnly === 'y') {
    const blob = `${wo.defectsFound || ''} ${wo.defectSummary || ''} ${wo.notes || ''}`.toLowerCase();
    if (!blob.includes('defect') && !blob.includes('issue') && !blob.includes('fail')) return false;
  }
...
/**
 * Applies `sortBy` / `sortDir` from the report query string to a mutating row array.
 * `sortBy`: `date` | `unit` | `cost` | `service` (default `date`). `sortDir`: `asc` | `desc` (default `desc`).
 */
export function sortRowsByReportQuery(rows, query, keyMap = {}) {
  if (!Array.isArray(rows) || !rows.length) return rows;
  const sortBy = String(query.sortBy || 'date').toLowerCase();
  const asc = String(query.sortDir || 'desc').toLowerCase() === 'asc';
  const mul = asc ? 1 : -1;
  const dateKey = keyMap.dateKey || 'date';
  const unitKey = keyMap.unitKey || 'unit';
  const costKey = keyMap.costKey || 'total';
  const serviceKey = keyMap.serviceKey || 'serviceType';
  const locationKey = keyMap.locationKey || 'location';
  const vendorKey = keyMap.vendorKey || 'vendor';
  rows.sort((a, b) => {
    if (sortBy === 'unit') return mul * String(a[unitKey] ?? '').localeCompare(String(b[unitKey] ?? ''), undefined, { numeric: true, sensitivity: 'base' });
    if (sortBy === 'cost')
      return mul * ((Number(a[costKey]) || 0) - (Number(b[costKey]) || 0));
    if (sortBy === 'service')
      return mul * String(a[serviceKey] ?? '').localeCompare(String(b[serviceKey] ?? ''), undefined, { numeric: true, sensitivity: 'base' });
    if (sortBy === 'location')
      return mul * String(a[locationKey] ?? '').localeCompare(String(b[locationKey] ?? ''), undefined, { numeric: true, sensitivity: 'base' });
    if (sortBy === 'vendor')
      return mul * String(a[vendorKey] ?? '').localeCompare(String(b[vendorKey] ?? ''), undefined, { numeric: true, sensitivity: 'base' });
...
export function buildVendorsUsedList(erp) {
  const set = new Set();
  for (const wo of activeWorkOrders(erp)) {
    const v = String(wo.vendor || '').trim();
    if (v) set.add(v);
  }
  return { vendors: [...set].sort((a, b) => a.localeCompare(b)) };
}

export function buildDriversUsedList(erp) {
  const set = new Set();
  for (const wo of activeWorkOrders(erp)) {
    const d = String(wo.driverName || '').trim();
    if (d) set.add(d);
  }
  return { drivers: [...set].sort((a, b) => a.localeCompare(b)) };
}
...
export async function buildReportDataset(id, erp, query, ctx = {}) {
  ...
  const statusFilter = String(query.status || '').trim().toLowerCase();
  const positionLineTokens = positionLineFilterTokens(query);
  ...
  switch (id) {
    case 'a1-work-order-history': {
      const gbRaw = String(query.groupBy || 'none').toLowerCase().replace(/-/g, '_');
      const gb = gbRaw === '' ? 'none' : gbRaw;
      const rows = [];
      for (const wo of activeWorkOrders(erp)) {
      ...
      sortRowsByReportQuery(rows, query, { costKey: 'total', serviceKey: 'serviceType' });
      ...
      const bucket = r => {
        if (gb === 'vehicle' || gb === 'unit') return String(r.unit || '').trim() || '—';
        if (gb === 'service_type' || gb === 'servicetype') return String(r.serviceType || '').trim() || '—';
        if (gb === 'record_type' || gb === 'recordtype') return String(r.recordCategory || '').trim() || '—';
        if (gb === 'location') return String(r.serviceLocation || '').trim() || '—';
        if (gb === 'vendor') return String(r.vendor || '').trim() || '—';
        if (gb === 'month') return (String(r.date || '').trim().slice(0, 7) || '—');
      ...
      for (const [key, list] of groups) {
        const tot = list.reduce((s, x) => s + (safeNum(x.total, 0) || 0), 0);
        sections.push({
          key,
          title: key,
          recordCategory: list[0] ? list[0].recordCategory : '',
          recordCount: list.length,
          totalCost: Math.round(tot * 100) / 100,
          avgCost: list.length ? Math.round((tot / list.length) * 100) / 100 : 0,
          columns: cols,
          rows: list
        });
      }
      ...
      return {
        ...
        columns: cols,
        rows: flatRows,
        totals: { count: rows.length, totalDollars: Math.round(sum * 100) / 100, sections: sections.length },
        meta: {
          ...
          chartXKey: 'title',
          chartYKey: 'totalCost',
          summaryCards: sections.slice(0, 12).map(s => ({
            label: s.title,
            value: `${s.recordCount} · $${Number(s.totalCost || 0).toFixed(2)}`
          }))
        }
      };
    }
    case 'a2-cost-by-unit': {
      const gb = String(query.groupBy || 'vehicle').toLowerCase().replace(/-/g, '_');
      const by = {};
      for (const wo of activeWorkOrders(erp)) {
        if (!workOrderPassesReportFilters(wo, query, ctx)) continue;
        const u = String(wo.unit || '').trim() || '—';
        ...
        let key = u;
        if (gb === 'make') {
          const mk = extractMakeFromYmm((fleetByUnit[u] && fleetByUnit[u].ymm) || '') || 'unknown';
          key = mk;
        } else if (gb === 'fleet_type' || gb === 'fleettype') {
          key = String(wo.fleetType || wo.vehicleType || wo.assetCategory || 'unspecified').trim() || 'unspecified';
        }
        if (!by[key]) by[key] = { key, units: new Set(), woCount: 0, total: 0, lastDate: '' };
        if (u && u !== '—') by[key].units.add(u);
        by[key].woCount += 1;
        by[key].total += t;
        const d = sliceIso(wo.serviceDate);
        if (d && d > by[key].lastDate) by[key].lastDate = d;
      }
      const rows = Object.values(by).map(r => {
        const unitList = [...r.units].filter(Boolean).sort();
        const sampleUnit = unitList[0] || '';
        return {
          groupKey: r.key,
          unit: gb === 'vehicle' ? r.key : unitList.slice(0, 3).join(', ') + (unitList.length > 3 ? '…' : ''),
          makeModel: (fleetByUnit[sampleUnit] && fleetByUnit[sampleUnit].ymm) || (gb === 'make' ? r.key : ''),
          fleetType: gb === 'fleet_type' || gb === 'fleettype' ? r.key : '',
          totalWOs: r.woCount,
      ...
      sortRowsByReportQuery(rows, query, {
        dateKey: 'lastServiceDate',
        unitKey: 'groupKey',
        costKey: 'totalDollars',
        serviceKey: 'makeModel'
      });
      const sumCost = rows.reduce((s, r) => s + (safeNum(r.totalDollars, 0) || 0), 0);
      const sumWo = rows.reduce((s, r) => s + (safeNum(r.totalWOs, 0) || 0), 0);
      const dimLabel = gb === 'make' ? 'Make' : gb === 'fleet_type' || gb === 'fleettype' ? 'Fleet type' : 'Unit';
      const chartX = gb === 'vehicle' ? 'unit' : 'groupKey';
      return {
        ok: true,
        title: 'Cost by unit',
        columns: [
          { key: 'groupKey', label: dimLabel },
          { key: 'unit', label: gb === 'vehicle' ? 'Unit' : 'Sample units' },
        ...
        meta: { hasChart: true, chartType: 'bar', chartXKey: chartX, chartYKey: 'totalDollars', totalRows: rows.length, groupBy: gb }
      };
    }
    case 'a3-cost-by-service-type': {
      const gb = String(query.groupBy || 'service_type').toLowerCase().replace(/-/g, '_');
      const by = {};
      for (const wo of activeWorkOrders(erp)) {
        if (!workOrderPassesReportFilters(wo, query, ctx)) continue;
        ...
        const t = woLineTotal(wo);
        let key = svc;
        if (gb === 'record_type' || gb === 'recordtype') key = recordCategoryKey(wo);
        else if (gb === 'category') key = serviceCategoryRollupLabel(svc);
        if (!by[key]) by[key] = { serviceType: key, count: 0, total: 0 };
        by[key].count += 1;
        by[key].total += t;
      }
      ...
      sortRowsByReportQuery(rows, query, { dateKey: '_date', unitKey: 'serviceType', costKey: 'totalDollars', serviceKey: 'serviceType' });
      const totCost = rows.reduce((s, r) => s + (safeNum(r.totalDollars, 0) || 0), 0);
      const totCnt = rows.reduce((s, r) => s + (safeNum(r.count, 0) || 0), 0);
      const dim =
        gb === 'record_type' || gb === 'recordtype'
          ? 'Record type'
          : gb === 'category'
            ? 'Category'
            : 'Service type';
      return {
        ...
        meta: {
          hasChart: true,
          chartType: 'pie',
          chartLabelKey: 'serviceType',
          chartYKey: 'totalDollars',
    ...
    case 'a4-pm-schedule': {
      ...
      const makeFilters = parseMultiParam(query, 'makes', 'make');
      const overdueOnly =
      ...
      for (const u of units) {
        ...
        if (makeFilters.length) {
          const mk = extractMakeFromYmm(fleet.ymm || '');
          const wantOther = makeFilters.some(m => String(m).toLowerCase() === 'all others');
          const known = ['mack', 'freightliner', 'peterbilt', 'volvo'];
          const isKnown = known.includes(mk);
          const ok = makeFilters.some(m => {
            const ml = String(m).trim().toLowerCase();
            if (ml === 'all others') return !isKnown;
            return mk && mk.includes(ml);
          });
        ...
        if (nextDue != null && cur != null && Number.isFinite(nextDue) && Number.isFinite(cur)) {
          ...
          if (mr < 0) approxMonthsRemainingSeverity = 'overdue';
          else if (mr <= 3000) approxMonthsRemainingSeverity = 'amber';
          else if (mr <= 8000) approxMonthsRemainingSeverity = 'blue';
          else approxMonthsRemainingSeverity = 'green';
          const milesPerWeek = fleetAvg / 4.33;
          if (milesPerWeek > 0) {
            const wk = mr / milesPerWeek;
            const wkAbs = Math.round(Math.abs(wk) * 10) / 10;
            if (mr < 0) {
              weeksRemainingDisplay = `${wkAbs.toFixed(1)} weeks overdue`;
            } else {
              weeksRemaining = Math.round(wk * 10) / 10;
              weeksRemainingDisplay = `${weeksRemaining.toFixed(1)} weeks`;
            }
          }
        }
        rows.push({
      ...
      const pmPick = parseMultiParam(query, 'pmStatus', 'pmStatuses');
      if (pmPick.length) {
        const allow = new Set(pmPick.map(x => String(x).trim().toLowerCase()));
        outRows = outRows.filter(r => allow.has(String(r.status || '').trim().toLowerCase()));
      }
      ...
      return {
        ...
        columns: [
          ...
          { key: 'weeksRemainingDisplay', label: 'Weeks remaining' },
          { key: 'approxMonthsRemaining', label: 'Approx. months remaining' },
          { key: 'approxMonthsRemainingSeverity', label: 'Miles band' },
        ...
        meta: {
          ...
          disclaimer: `Fleet average pace: ${fleetAvg.toLocaleString(
            'en-US'
          )} mi per truck per month. PM colors: green if miles remaining > 8,000; blue if 3,000–8,000; amber if 0–3,000; red if overdue. Weeks ≈ miles remaining ÷ (${(fleetAvg / 4.33).toFixed(0)} mi/wk). Catalog month intervals use FLOOR(miles ÷ fleet avg), minimum 1 month (plus fixed overrides for select slugs).`
        }
      };
    }
    case 'a5-tire-history': {
      const rows = [];
      for (const wo of activeWorkOrders(erp)) {
        if (!workOrderPassesReportFilters(wo, queryWithoutPositionKeys(query), ctx)) continue;
        for (const ln of wo.lines || []) {
          const isTire =
          ...
          if (!lineMatchesPositionLineTokens(pos, positionLineTokens)) continue;
          rows.push({
            date: sliceIso(wo.serviceDate),
            unit: wo.unit || '',
            position: pos || '—',
      ...
      sortRowsByReportQuery(rows, query, { costKey: 'cost', serviceKey: 'description' });
      const cols = [
        { key: 'date', label: 'Date' },
        { key: 'unit', label: 'Unit' },
      ...
      const gb = String(query.groupBy || 'none').toLowerCase().replace(/-/g, '_');
      if (gb === 'none' || gb === 'flat' || gb === '') {
        return {
          ok: true,
          title: 'Tire history',
          columns: cols,
          rows,
          meta: { positions: [...new Set(rows.map(r => r.position).filter(p => p && p !== '—'))].sort(), groupBy: gb }
        };
      }
      const bucket = r => {
        if (gb === 'position') return String(r.position || '—').trim() || '—';
        if (gb === 'vehicle' || gb === 'unit') return String(r.unit || '—').trim() || '—';
        if (gb === 'date') return String(r.date || '—').slice(0, 10) || '—';
        return '—';
      };
      const groups = new Map();
      for (const r of rows) {
      ...
      const sections = [];
      for (const [key, list] of groups) {
        const tot = list.reduce((s, x) => s + (safeNum(x.cost, 0) || 0), 0);
        sections.push({
          key,
          title: key,
          recordCategory: 'tire',
      ...
      return {
        ...
        meta: {
          ...
          sections,
          groupBy: gb,
          positions: [...new Set(rows.map(r => r.position).filter(p => p && p !== '—'))].sort(),
          hasChart: true,
          chartType: 'bar',
          chartSource: 'sections',
          chartXKey: 'title',
    ...
    case 'a6-air-bag-history': {
      ...
      if (gb === 'none' || gb === 'flat' || gb === '') {
        return {
          ...
          columns: cols,
          rows,
          meta: { positions: [...new Set(rows.map(r => r.position))].filter(Boolean).sort(), groupBy: gb }
        };
      }
      const bucket = r => {
        if (gb === 'position') return String(r.position || '—').trim() || '—';
      ...
      return {
        ...
        meta: {
          ...
          sections,
          groupBy: gb,
          positions: [...new Set(rows.map(r => r.position))].filter(Boolean).sort(),
          hasChart: true,
          chartType: 'bar',
          chartSource: 'sections',
          chartXKey: 'title',
    ...
    case 'a7-battery-history': {
      ...
      for (const wo of activeWorkOrders(erp)) {
        ...
        for (const ln of wo.lines || []) {
          const posRaw = String(ln.partPosition || ln.tirePosition || ln.tirePositionText || '').trim();
          if (!lineMatchesPositionLineTokens(posRaw, positionLineTokens)) continue;
          const pos = posRaw || '—';
          rows.push({
            date: sliceIso(wo.serviceDate),
            unit: wo.unit || '',
    ...
    case 'a8-accident-collision': {
      ...
      sortRowsByReportQuery(rows, query, { costKey: 'actualRepair', serviceKey: 'location' });
      return {
        ok: true,
    ...
    case 'a9-fleet-repair-monthly': {
      ...
      for (const wo of activeWorkOrders(erp)) {
        ...
        if (!d) continue;
        if (!workOrderPassesReportFilters(wo, query, ctx)) continue;
        const bk = bucketKey(d);
        if (!bk) continue;
        if (!by[bk]) by[bk] = { period: bk, woCount: 0, total: 0, svcCounts: {} };
        by[bk].woCount += 1;
        by[bk].total += woLineTotal(wo);
        const svc = primaryServiceType(wo);
        by[bk].svcCounts[svc] = (by[bk].svcCounts[svc] || 0) + 1;
      }
      ...
      const periodLabel = grain === 'year' ? 'Year' : grain === 'quarter' ? 'Quarter' : 'Month';
      return {
        ok: true,
        title: 'Fleet repair summary (monthly)',
        columns: [
          { key: 'month', label: periodLabel },
        ...
        rows,
        meta: { hasChart: true, chartType: 'line', chartXKey: 'month', chartYKey: 'totalDollars', groupBy: grain }
      };
    }
    case 'a10-inspection-history': {
      ...
      sortRowsByReportQuery(rows, query, { serviceKey: 'inspection_type' });
      const cols = [
        { key: 'date', label: 'Date' },
        { key: 'unit', label: 'Unit' },
      ...
      if (gb === 'none' || gb === 'flat' || gb === '') {
        return {
          ...
          columns: cols,
          rows,
          meta: { totalRows: rows.length, groupBy: gb }
        };
      }
      const bucket = r => {
        if (gb === 'position') return String(r.inspection_type || '—').trim().slice(0, 60) || '—';
        if (gb === 'vehicle' || gb === 'unit') return String(r.unit || '—').trim() || '—';
        if (gb === 'date') return String(r.date || '—').slice(0, 10) || '—';
        return '—';
      };
      ...
      sections.sort((a, b) => b.recordCount - a.recordCount);
      return {
        ok: true,
        title: 'Inspection history',
        columns: cols,
        ...
        meta: {
          ...
          summaryCards: sections.slice(0, 12).map(s => ({
            label: s.title,
            value: `${s.recordCount} records`
          }))
        }
      };
    }
    case 'a11-parts-positions': {
      const rows = [];
      for (const wo of activeWorkOrders(erp)) {
        if (!workOrderPassesReportFilters(wo, queryWithoutPositionKeys(query), ctx)) continue;
        for (const ln of wo.lines || []) {
          const pos = String(ln.tirePosition || ln.tirePositionText || ln.partPosition || '').trim();
          const pn = String(ln.partNumber || ln.partNo || '').trim();
          const lt = String(ln.lineType || '').toLowerCase();
          const hasPartSignal =
            Boolean(pos) ||
            Boolean(pn) ||
            lt === 'tire' ||
            lt === 'item' ||
            Boolean(String(ln.sku || '').trim()) ||
            Boolean(String(ln.qboItemId || '').trim());
          ...
          rows.push({
            ...
            woNumber: wo.workOrderNumber || wo.internalWorkOrderNumber || wo.id,
            recordCategory: recordCategoryKey(wo),
            serviceType: String(ln.serviceType || primaryServiceType(wo) || '').trim() || '—',
            position: pos || '—',
            partNumber: pn || '—',
            sku: String(ln.sku || '').trim(),
            description: String(ln.notes || ln.description || '').trim(),
            mileage: ln.serviceMileage ?? '',
            amount: Math.round(safeNum(ln.amount, 0) * 100) / 100,
            vendor: String(wo.vendor || '').trim(),
            serviceLocation: String(wo.repairLocationLabel || wo.repairLocationType || '').trim()
          });
        }
      }
      sortRowsByReportQuery(rows, query, { costKey: 'amount', serviceKey: 'serviceType' });
      const cols = [
        { key: 'date', label: 'Date' },
        { key: 'unit', label: 'Unit' },
        { key: 'woNumber', label: 'WO#' },
      ...
      if (gb === 'none' || gb === 'flat' || gb === '') {
        return {
          ok: true,
          title: 'Parts / positions (line detail)',
          columns: cols,
          rows,
          meta: {
            positions: [...new Set(rows.map(r => r.position).filter(p => p && p !== '—'))].sort(),
      ...
      const sections = [];
      for (const [key, list] of groups) {
        const tot = list.reduce((s, x) => s + (safeNum(x.amount, 0) || 0), 0);
        sections.push({
          key,
          title: key,
          recordCategory: list[0] ? list[0].recordCategory : '',
      ...
      return {
        ...
        meta: {
          ...
          positions: [...new Set(rows.map(r => r.position).filter(p => p && p !== '—'))].sort(),
          totalRows: rows.length,
          summaryCards: sections.slice(0, 14).map(s => ({
            label: s.title,
            value: `${s.recordCount} · $${Number(s.totalCost || 0).toFixed(2)}`
          }))
        }
    ...
    case 'b6-expense-history': {
      ...
      sortRowsByReportQuery(rows, query, { costKey: 'amount', serviceKey: 'vendor' });
      return {
        ok: true,
    ...
    case 'b8-fuel-expense-history': {
      ...
      sortRowsByReportQuery(rows, query, { costKey: 'total', serviceKey: 'vendor' });
      return {
        ok: true,
    ...
    case 'd2-fuel-cost-by-driver': {
      ...
      sortRowsByReportQuery(rows, query, { dateKey: '_d', unitKey: 'driver', costKey: 'totalDollars', serviceKey: 'driver' });
      return {
        ok: true,
    ...
    case 'm1-expense-by-service-type': {
      ...
      for (const [key, list] of groups) {
        ...
        const recCat = recordCategoryKey(list[0]);
        sections.push({
          key,
          title: key,
          recordCategory: recCat,
          categoryPill: groupBy === 'service_type' || groupBy === 'servicetype' ? catHint : recCat,
          recordCount: list.length,
    ...
    case 'm4-work-by-location': {
      ...
      for (const [key, list] of groups) {
        ...
        const { locationPill: locPill, pillTone } = dominantServiceLocationPill(list);
        const subRows = list
          .map(wo => {
            const u = String(wo.unit || '').trim();
            const fleet = fleetByUnit[u] || {};
        ...
        const svcMixMap = new Map();
        for (const wo of list) {
          const k = primaryServiceType(wo) || '—';
          svcMixMap.set(k, (svcMixMap.get(k) || 0) + woLineTotal(wo));
        }
        const serviceTypeMix = [...svcMixMap.entries()]
          .map(([name, tot]) => ({ name, total: Math.round(tot * 100) / 100 }))
          .sort((a, b) => b.total - a.total)
          .slice(0, 6);
        sections.push({
          key,
          title: key,
          recordCategory: recordCategoryKey(list[0]),
    ...
    case 'm5-internal-external': {
      ...
      const isRoad = wo => {
        const t = String(wo.repairLocationType || '').toLowerCase();
        return t === 'road-service' || t === 'roadside' || t === 'road_side';
      };
      ...
      const extVendors = byVendor(external);
      const topExt = extVendors[0];
      const inHouseHint =
        topExt && topExt.totalCost > 0
          ? `Largest external vendor by spend: ${topExt.vendor} ($${topExt.totalCost.toFixed(2)}). Consider whether recurring work could be phased in-house where it makes sense.`
          : '';
      const nRec = wos.length || 1;
      const pctIntRec = Math.round((internal.length / nRec) * 10000) / 100;
      const pctExtRec = Math.round((external.length / nRec) * 10000) / 100;
      const pctRoadRec = Math.round((road.length / nRec) * 10000) / 100;
      return {
        ok: true,
        ...
        meta: {
          ...
          roadside: { rows: bySvc(road), title: 'Roadside services' },
          compareSpend: {
          ...
          summaryCards: [
            ...
            { label: 'Mix (records)', value: `In ${pctIntRec}% · Ex ${pctExtRec}% · Rd ${pctRoadRec}%` }
          ],
          inHouseHint,
    ...
    case 'm6-location-summary': {
      ...
      sortRowsByReportQuery(rows, query, {
        dateKey: 'lastVisit',
        unitKey: 'locationName',
        costKey: 'totalCost',
        serviceKey: 'mostCommonService'
      });
      return {
        ok: true,
