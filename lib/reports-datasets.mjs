...
export async function buildReportDataset(id, erp, query, ctx = {}) {
  ...
  switch (id) {
    ...
    case 'c10-dvir': {
      const titles = {
        'c2-hos-violations': 'HOS violations log',
        'c3-daily-driver-log': 'Daily driver log summary',
        'c4-safety-score-driver': 'Safety score by driver',
        'c5-safety-score-fleet': 'Safety score fleet overview',
        'c6-speeding': 'Speeding report',
        'c7-harsh-driving': 'Harsh driving events',
        'c8-unassigned-hos': 'Unassigned HOS segments',
        'c10-dvir': 'DVIR (Driver Vehicle Inspection Reports)'
      };
      return {
        ok: true,
        title: titles[id] || 'Samsara report',
        disclaimer: ctx.samsaraConnected
          ? 'Detailed rows require additional Samsara Safety / HOS history endpoints; this build shows the report shell with live connection.'
          : 'Connect Samsara (SAMSARA_API_TOKEN) to enable telematics-backed safety and HOS reports.',
        columns: [{ key: 'message', label: 'Note' }],
        rows: [{ message: 'Use Refresh on Overview after connecting Samsara; extend API mapping for full row history.' }]
      };
    }
    ...
    case 'g3-drug-alcohol-testing': {
      return {
        ...
        disclaimer:
          'FMCSA testing events are not yet persisted as first-class records. Extend driver profiles or add a testing log collection to populate this grid.',
        columns: [
          { key: 'driver', label: 'Driver' },
          ...
          { key: 'lab', label: 'Lab' },
          { key: 'mro', label: 'MRO' },
          { key: 'nextRandom', label: 'Next random due' }
        ],
        rows: []
      };
    }
