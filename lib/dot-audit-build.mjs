...
import { workOrderPassesReportFilters } from './reports-datasets.mjs';
...
export function buildDotAuditJson(erp, unit, startDate, endDate, companyProfile = {}, queryOpts = {}, ctx = {}) {
  ...
  let wos = activeWos(erp).filter(w => String(w.unit || '').trim() === u && inRange(w.serviceDate));
  const qf = { ...(queryOpts && typeof queryOpts === 'object' ? queryOpts : {}), units: u, unit: u };
  const fleetCtx = ctx && typeof ctx === 'object' ? ctx : { fleetByUnit: {} };
  wos = wos.filter(w => workOrderPassesReportFilters(w, qf, fleetCtx));
