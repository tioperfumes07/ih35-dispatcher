...
export function parseRateConfirmationText(raw) {
  ...
  let customerWoNumber = null;
  const woPatterns = [
    /\b(?:W\/O|WO)\s*#?\s*:?\s*([A-Z0-9][A-Z0-9\-]{2,24})\b/i,
    /\bWork\s*Order\s*#?\s*:?\s*([A-Z0-9][A-Z0-9\-]{2,24})\b/i,
    /\bCustomer\s*(?:WO|W\/O)\s*#?\s*:?\s*([A-Z0-9][A-Z0-9\-]{2,24})\b/i,
    /\bOrder\s*ref(?:erence)?\s*#?\s*:?\s*([A-Z0-9][A-Z0-9\-]{2,24})\b/i
  ];
  for (const re of woPatterns) {
    const m = single.match(re);
    if (m) {
      const cand = String(m[1] || '').trim();
      if (cand && cand.toLowerCase() !== String(loadNumber || '').toLowerCase()) {
        customerWoNumber = cand.slice(0, 80);
        hints.push('customer_wo_pattern');
        break;
      }
    }
  }
