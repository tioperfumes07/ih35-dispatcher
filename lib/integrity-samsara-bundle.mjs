...
import { samsaraPaginate } from './samsara-report-fetch.mjs';
import {
...
export async function buildVehicleIntegrityBundle(opts) {
  ...
  if (vehicleId && hasSamsaraTokenFromEnv()) {
  ...
  function hasSamsaraTokenFromEnv() {
...
function monthlyMaintenanceCostSeries(erp, unit, months) {
  ...
  for (const wo of erp?.workOrders || []) {
    ...
    const amt = Number(wo.totalCost || wo.amount || 0) || 0;
    buckets[d] = (buckets[d] || 0) + amt;
    for (const ln of wo.lines || []) {
      buckets[d] = (buckets[d] || 0) + (Number(ln.amount) || 0);
    }
  }
