...
export async function buildDotVehicleAuditV1(erp, unitId, startDate, endDate, companyProfile = {}, queryOpts = {}, ctx = {}) {
  ...
  const keepEmptySection4 = ['1', 'true', 'yes'].includes(String(queryOpts.keepEmptySection4Buckets || '').toLowerCase());
  ...
  if (!includeEmpty) {
    for (const k of Object.keys(sectionsOut)) {
      if (k === 'section4_location_summary') continue;
      if (keepEmptySection4 && WO_BUCKET_KEYS.includes(k)) continue;
  ...
  return {
    ...
    meta: {
      ...
      keepEmptySection4Buckets: keepEmptySection4,
      disclaimer:
