...
export function mergeIntegrityThresholds(overrides = {}) {
  ...
  if (overrides && typeof overrides === 'object' && !Array.isArray(overrides)) {
    if (overrides.integrityThresholds && typeof overrides.integrityThresholds === 'object') {
      extra = overrides.integrityThresholds;
    } else if (overrides.maxTiresPerUnit90d != null || overrides.maxDriverSpend90d != null) {
      extra = overrides;
    }
  }
  return { ...defs, ...extra };
...
export function evaluateIntegrityCheck(ctx = {}, erp = {}) {
  ...
  if (unit) {
    ...
    for (const r of records) {
      ...
      const d = recDate(r);
      if (!d) continue;
      if (daysBetweenIso(d, ctxDay || d) <= 90 || isWithinDays(d + 'T12:00:00Z', 90, now)) {
    ...
    for (const w of workOrders) {
      ...
      const d = woDate(w);
      if (!d) continue;
      if (isWithinDays(d + 'T12:00:00Z', 90, now)) {
    ...
    for (const r of records) {
      ...
      const d = recDate(r);
      if (!d || !isWithinDays(d + 'T12:00:00Z', 180, now)) continue;
  ...
  if (driverId) {
    ...
    for (const r of records) {
      ...
      const d = recDate(r);
      if (d && isWithinDays(d + 'T12:00:00Z', 90, now)) {
        spend90 += recordCost(r);
        if (isRepairish(r)) repairs++;
        if (isAccidentish(r)) accids++;
    ...
    for (const w of workOrders) {
      ...
      if (d && isWithinDays(d + 'T12:00:00Z', 90, now)) {
        ...
        if (mrt === 'accident' || isAccidentish(w)) accids++;
    ...
    for (const r of records) {
      if (String(r.driverId || '') !== driverId) continue;
      const d = recDate(r);
      if (d && isWithinDays(d + 'T12:00:00Z', 365, now) && isAccidentish(r)) accids++;
    }
    for (const w of workOrders) {
      if (String(w.driverId || '') !== driverId) continue;
      const d = woDate(w);
      if (d && isWithinDays(d + 'T12:00:00Z', 365, now) && isAccidentish(w)) accids++;
    }
    ...
    if (unit) {
      ...
      for (const x of driverUnit) {
        if (x.u === unit && x.t && isWithinDays(x.t + 'T12:00:00Z', 60, now)) pair++;
      }
  ...
  if (unit) {
    ...
    for (const r of records) {
      ...
      const d = recDate(r);
      if (!d || !isWithinDays(d + 'T12:00:00Z', 365, now)) continue;
      accN++;
    ...
    for (const r of records) {
      if (String(r.unit || '') !== unit || !isAccidentish(r)) continue;
      const d = recDate(r);
      if (!d || !isWithinDays(d + 'T12:00:00Z', 90, now)) continue;
      qCost += recordCost(r);
    }
    for (const w of workOrders) {
      if (String(w.unit || '') !== unit || !isAccidentish(w)) continue;
  ...
  if (isAccidentish(ctx) && ctx.dotReportable) {
    pushAlert(
      'A3',
      'RED',
  ...
  if (unit) {
    ...
    for (const r of records) {
      if (String(r.unit || '') !== unit) continue;
      const d = recDate(r);
      if (!d || !isWithinDays(d + 'T12:00:00Z', 60, now)) continue;
      svc60++;
    }
...
export function effectiveIntegrityAlertCategory(alert = {}) {
  if (alert.category && String(alert.category).trim()) {
    const c = String(alert.category).toLowerCase();
