...
export function defaultIntegrityThresholds() {
  return {
    ...
    maxMonthlyCostPerUnit: 3000,
    maxServiceRecordsPerUnit60d: 5,
      ...
      if (unit) {
        ...
        if (monthCost > th.maxMonthlyCostPerUnit) {
          push(
            ...
            `Unit ${unit} has $${monthCost.toFixed(
              2
            )} in maintenance costs this month. This is above the normal threshold. Consider whether this unit needs retirement or major overhaul evaluation.`,
            { unit, monthCost, threshold: th.maxMonthlyCostPerUnit },
        ...
        if (svcCount >= th.maxServiceRecordsPerUnit60d) {
          push(
            'M3',
            'AMBER',
            `Unit ${unit} has had ${svcCount} service records in the last 60 days. This unit may have an underlying mechanical issue requiring inspection.`,
            { unit, count: svcCount, threshold: th.maxServiceRecordsPerUnit60d },
            `M3:${unit}`
          );
        }
