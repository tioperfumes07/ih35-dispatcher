 ...
 *   `TxnDate` on/after (today − N years) are scanned. Omit or invalid = use a wide default
 *   `TxnDate >= 1990-01-01` on every page (some QBO companies reject unpaged/unbounded Purchase
 *   queries with QueryValidationError mentioning EntityRef).
...
/** Lower bound when env lookback is unset; keeps Purchase queries filterable in strict QBO tenants. */
const PURCHASE_TXN_DATE_FALLBACK_MIN = '1990-01-01';
...
/** Effective `TxnDate` lower bound for Purchase queries (never rely on unfiltered Purchase SELECT). */
function purchaseTxnDateLowerBound(cutoffIso) {
  return cutoffIso && /^\d{4}-\d{2}-\d{2}$/.test(cutoffIso) ? cutoffIso : PURCHASE_TXN_DATE_FALLBACK_MIN;
}
...
export function getQboDedupePurchaseScanTuning() {
  ...
  return {
    ...
    effectivePurchaseTxnMinIso: purchaseTxnDateLowerBound(cfg.cutoffIso),
...
async function listPurchaseTxnIdsForVendorUncached(qboQuery, vendorId, cutoffIso) {
  ...
  let start = 1;
  const pageSize = 500;
  const txnMin = purchaseTxnDateLowerBound(cutoffIso);
  const whereTxn = ` where TxnDate >= '${txnMin}'`;
  while (true) {
