...
/** Writable data root (persist volume or local ./data). */
export const ERP_DATA_DIR = DATA_DIR;
...
/** Map QBO vendor id → { name, companyName } from cached QuickBooks sync (erp.json). */
export function readQboVendorLookup() {
  try {
    const raw = JSON.parse(fs.readFileSync(ERP_FILE, 'utf8'));
    const vendors = raw?.qboCache?.vendors || [];
    const map = new Map();
    for (const v of vendors) {
      if (v?.qboId != null) map.set(String(v.qboId), v);
    }
    return map;
  } catch {
    return new Map();
