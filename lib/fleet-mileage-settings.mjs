 ...
 * Catalog `interval_months` uses ROUND(miles / avg), minimum 1, plus slug overrides (see `applyFleetCatalogMonthOverrides`).
 */
...
/**
 * Months from miles at given fleet average (ROUND, minimum 1 when miles > 0).
...
export function milesToRoundMonths(miles, avgPerMonth) {
  ...
  return Math.max(1, Math.round(m / a));
}

/** @deprecated Use milesToRoundMonths — kept for older imports/tests */
export const milesToFloorMonths = milesToRoundMonths;

/** After ROUND-based recompute, align catalog rows with fleet scheduling overrides. */
...
/** Recompute stored month columns from mileage columns at a new fleet average (ROUND, min 1). */
export async function recalcAllIntervalMonthsFromFleetAvg(dbQuery, avg) {
  ...
  await dbQuery(
    `UPDATE service_types SET interval_months = CASE
       ...
       ELSE GREATEST(1, ROUND(interval_miles::numeric / $1))
     END`,
    [a]
  );
  await dbQuery(
  ...
  await dbQuery(
    `UPDATE vehicle_parts_reference SET avg_replacement_months = CASE
       ...
       ELSE GREATEST(1, ROUND(avg_replacement_miles::numeric / $1))
     END`,
    [a]
  );
...
export function formatIntervalDualLine(intervalMiles, intervalMonths, fleetAvg) {
  ...
  if (mi != null) {
    const est = milesToRoundMonths(mi, fleetAvg);
    return {
      milesLine: `${mi.toLocaleString('en-US')} mi`,
