...
export function buildDueRowsForProfileFixed(profile, schedules, erp, partsRows) {
...
export async function getVehicleDueList(dbQuery, erp, enrichedVehicles, unitId) {
  ...
  const items = buildDueRowsForProfileFixed(profile, schedules, erp, partsRows);
  ...
  for (const r of fleet.rows || []) {
    ...
    if (r.status === 'OVERDUE') {
      ...
      alerts.push({
        ...
        severity: 'RED',
        unitId: u,
        message: `Unit ${u} is overdue for ${svc}${mo != null ? ` by ${mo} mi` : ''}${dy != null ? ` / ${dy} d` : ''}. Manufacturer baseline: every ${r.interval_miles || '—'} mi / ${r.interval_months || '—'} mo.`,
      ...
      alerts.push({
        ...
        severity: 'AMBER',
        unitId: u,
        message: `Unit ${u} is due soon for ${svc}: ${r.miles_remaining != null ? Math.round(r.miles_remaining) + ' mi' : '—'} / ${r.days_remaining != null ? Math.round(r.days_remaining) + ' d' : '—'} remaining.`,
