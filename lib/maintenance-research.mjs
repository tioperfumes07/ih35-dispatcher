    ...
    cards.push({
      ...
      chart: {
        ...
        industry_avg_monthly: labels.map(() =>
          Math.round(industryMaintPerMi * fleetAvg * n * 100) / 100
        )
      }
    });
  }
  cards.sort((a, b) => String(a.make).localeCompare(String(b.make)));
  return { ok: true, window_days: windowDays, fleet_avg_miles_per_month: fleetAvg, cards };
}
...
export async function evaluatePredictiveMaintenanceAlerts(dbQuery, erp, enrichedVehicles) {
  ...
  for (const r of fleet.rows || []) {
    ...
    if (r.status === 'OVERDUE') {
      ...
      const remain = pace ? ` (${pace} remaining)` : '';
      alerts.push({
