...
export async function ensureAppDatabaseObjects() {
  ...
  console.log('[db] Ensuring required tables (critical DDL: merge_log, dedupe, rename, integrity, schedules, fleet)…');
  await runDdlList(CRITICAL_DDL);
  ...
  if (missing.length) {
    throw new Error(
      `[db] Required tables missing after initialization: ${missing.join(', ')}. Fix database permissions or errors above.`
    );
  }

  console.log('[db] All required support tables present (merge_log, dedupe, rename, fleet catalog, schedules, company_settings).');
}
