...
/** @typedef {(text: string, params?: unknown[]) => Promise<{ rows?: unknown[] }>} PgQueryFn */

/**
 * @param {string[]} tableNames
 * @param {string} label
 * @param {PgQueryFn} [queryFn]
 */
async function assertTablesPresent(tableNames, label, queryFn = dbQuery) {
...
/** @param {PgQueryFn} [queryFn] */
async function seedDefaultSettings(queryFn = dbQuery) {
 ...
 * @param {PgQueryFn} [queryFn]
 */
async function runDdlList(statements, queryFn = dbQuery) {
  for (const sql of statements) {
    const trimmed = stripLeadingComments(sql);
    if (!trimmed) continue;
    await queryFn(trimmed);
  }
}
...
export async function ensureDedupeWritePathObjects() {
  ...
  try {
    // One session: avoids CREATE landing outside `public` while merges read `public.merge_log`.
    await client.query('SET search_path TO public');
    /** @type {PgQueryFn} */
    const q = (text, params) => client.query(text, params);

    console.log('[db] merge audit path: CREATE IF NOT EXISTS merge_log, dedup_skipped (session, public schema)…');
    await runDdlList(MERGE_AUDIT_DDL, q);
    await assertTablesPresent(['merge_log', 'dedup_skipped'], 'Merge audit / dedupe skip', q);
