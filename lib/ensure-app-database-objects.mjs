...
/**
 * Core DDL for dedupe merge, skip list, rename tools, integrity mirror, scheduled_reports mailbox,
 * company_settings, and schema_migrations. merge_log is first so a partial failure never leaves
 * merge without its audit table.
 */
const CORE_DEDUPE_DDL = [
...
async function runDdlList(statements) {
...
export async function ensureDedupeWritePathObjects() {
  if (!getPool()) return;
  await runDdlList(CORE_DEDUPE_DDL);
...
export async function ensureAppDatabaseObjects() {
  ...
  const fatalMissing = missing.filter(n => !FLEET_CATALOG_TABLE_NAMES.has(n));
  if (fatalMissing.length) {
    throw new Error(
      `[db] Required tables missing after initialization: ${fatalMissing.join(', ')}. Fix database permissions or errors above.`
    );
  }
  if (missing.length) {
    console.error(
      '[db] Fleet catalog tables missing (QBO merge still available):',
      missing.join(', '),
      '— enable pgcrypto or fix DDL errors above.'
    );
  }

  console.log(
    '[db] Core support tables ready (merge_log, dedupe, rename, schedules, company_settings' +
      (missing.length ? '; fleet catalog tables incomplete — see errors above' : '; fleet catalog OK') +
      ').'
  );
}
