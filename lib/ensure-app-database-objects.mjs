...
export const APP_SUPPORT_TABLE_NAMES = [
  ...
  'relay_card_assignments',
  'relay_webhook_events',
  'dip_bank_account_balances',
  'banking_transactions',
  'bank_account_preferences',
  'banking_rules',
  'bank_txn_links',
  'driver_settlements',
  'driver_settlement_loads',
  'factoring_advances',
  'factoring_advance_loads',
  'bank_import_batches',
...
CREATE TABLE IF NOT EXISTS public.schema_migrations (
  ...
  `-- 024_canonical_schema_core_tables.sql
...
CREATE TABLE IF NOT EXISTS public.equipment_assignments (
  ...
  ON public.equipment_assignments (equipment_id, status, assigned_date DESC)`,

  `CREATE INDEX IF NOT EXISTS idx_equipment_assignments_unit
  ON public.equipment_assignments (unit_number, status, assigned_date DESC
)`,

  `-- 026_relay_webhook_foundation.sql
CREATE TABLE IF NOT EXISTS public.relay_card_assignments (
  id SERIAL PRIMARY KEY,
  ...
  ON public.relay_card_assignments (card_last4)
  WHERE active = true`,

  `CREATE INDEX IF NOT EXISTS idx_relay_card_assignments_unit
  ON public.relay_card_assignments (unit_number, active)`,

  `CREATE TABLE IF NOT EXISTS public.relay_webhook_events (
  id SERIAL PRIMARY KEY,
  ...
  ON public.relay_webhook_events (external_event_id)`,

  `CREATE UNIQUE INDEX IF NOT EXISTS uq_relay_webhook_events_payload_hash
  ON public.relay_webhook_events (payload_hash)`,
  `-- 027_banking_tables.sql
CREATE TABLE IF NOT EXISTS public.dip_bank_account_balances (
  id SERIAL PRIMARY KEY,
  ...
  ON public.banking_transactions (account_id, txn_date DESC)`,
  `CREATE INDEX IF NOT EXISTS idx_banking_transactions_source
  ON public.banking_transactions (source, txn_date DESC)`,
  `-- 028_banking_foundation.sql
CREATE TABLE IF NOT EXISTS public.bank_account_preferences (
  id BIGSERIAL PRIMARY KEY,
  ...
  `CREATE TABLE IF NOT EXISTS public.bank_txn_links (
  id BIGSERIAL PRIMARY KEY,
  txn_id BIGINT NOT NULL REFERENCES public.banking_transactions(id) ON DELETE CASCADE,
  entity_type TEXT NOT NULL,
  ...
  paid_date DATE,
  bank_txn_id BIGINT REFERENCES public.banking_transactions(id) ON DELETE SET NULL,
  notes TEXT,
  created_by TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
  ...
  id BIGSERIAL PRIMARY KEY,
  settlement_id BIGINT NOT NULL REFERENCES public.driver_settlements(id) ON DELETE CASCADE,
  load_number TEXT NOT NULL,
  gross_component NUMERIC(14,2),
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
)`,
  `CREATE TABLE IF NOT EXISTS public.factoring_advances (
  id BIGSERIAL PRIMARY KEY,
  ...
  `CREATE TABLE IF NOT EXISTS public.factoring_advance_loads (
  id BIGSERIAL PRIMARY KEY,
  factoring_advance_id BIGINT NOT NULL REFERENCES public.factoring_advances(id) ON DELETE CASCADE,
  load_number TEXT NOT NULL,
  invoice_number TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
)`,
  ...
  id BIGSERIAL PRIMARY KEY,
  reconciliation_session_id BIGINT NOT NULL REFERENCES public.bank_reconciliation_sessions(id) ON DELETE CASCADE,
  banking_txn_id BIGINT NOT NULL REFERENCES public.banking_transactions(id) ON DELETE CASCADE,
  ...
  ON public.banking_transactions (status)`,
  `CREATE INDEX IF NOT EXISTS idx_banking_rules_active_priority
  ON public.banking_rules (active, priority)`,
  `CREATE INDEX IF NOT EXISTS idx_driver_settlements_driver_period
  ON public.driver_settlements (driver_id, period_start)`,
  `CREATE INDEX IF NOT EXISTS idx_bank_recon_sessions_account_month
  ON public.bank_reconciliation_sessions (account_id, statement_month)`
];
...
const FLEET_CATALOG_DDL = [
  ...
  `CREATE INDEX IF NOT EXISTS idx_fleet_assets_unit_number ON public.fleet_assets (unit_number)`
];
