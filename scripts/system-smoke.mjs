 ...
 * Run: start the server, then `npm run smoke`. For **Rule 0 guard + layout test + smoke** in one step: `npm run qa:automated` (**`smoke:gate-sync`** + **`rule0:check`** + **`test:layout`** + this script; requires server up) or `npm run qa:isolated` (**`scripts/smoke-gate-paths-sync.mjs`** first, then a temp **`server.js`** on a free port with **`IH35_SMOKE_GATE=1`** from **`qa-with-server.mjs`**, then **`rule0:check`** + **`test:layout`** + this script + **`test:name-mgmt`** + **`test:fleet-mileage`**).
 ...
 * plus static CSS/JS (design-tokens, app-theme, erp-master-redesign, erp-master-spec-2026, maint-accounting, board-nav.css, erp-responsive-global-2026.css, erp-ui.js, board-nav.js) for HTTP 200 + stable header needles.
